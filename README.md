...
![LSB bytes order problem](extras/lsb_problem.png)
...
```
> avrdude -p atmega328p -c arduino -P COM8 -b 115200 -D -U flash:w:kpn532_foo.hex:i
```
...
```
> dfu-util --serial 370B0B185931393864FA33324B572E3F --alt 0 --reset --download kpn532_foo.ino.bin
dfu-util 0.11
...
![Picture of Arduino UNO R4 minima and two Elechouse NFC module v3, on a kpn532 board with level shifter](extras/kpn532_board_r4.jpg)

### Previous version with wires and previous pinning
![Picture of Arduino UNO R3 and two Elechouse NFC module v3, wired to SPI bus & powered](extras/kpn532_direct.jpg)
...
![Measured waiting time for GetVersion instruction on relayed configuration, WTX frames before answer](extras/kpn532_proxified_getversion.png)

A normal _waiting_ time is around ~383µs for this instruction
![Measured waiting time for GetVersion instruction on direct configuration](extras/kpn532_direct_getversion.png)
...
```
  .#####.         kirelay 0.2 (Arduino with kpn532 - single board)
 .## ^ ##.__ _    "A La Vie, A L'Amour" - (oe.eo)
 ## / \ /   ('>-
 ## \ / | K  |    /*** Benjamin DELPY `gentilkiwi`
 '## v #\____/         benjamin.delpy@banque-france.fr
  '#####' L\_          benjamin@gentilkiwi.com         ***/

| Relay verbosity: minimal
| Relay mode     : ISO/DEP
| PN532 #0 - Reader init...
|   PN532 version 1.6.7
|   CIU Version 0x80
| PN532 #1 - Emulator init...
|   PN532 version 1.6.7
|   CIU Version 0x80
| Initialization OK!
~ Waiting for target...
...
```
  .#####.         st25tb 0.1 (Arduino with kpn532)
 .## ^ ##.__ _    "A La Vie, A L'Amour" - (oe.eo)
 ## / \ /   ('>-
 ## \ / | K  |    /*** Benjamin DELPY `gentilkiwi`
 '## v #\____/         benjamin.delpy@banque-france.fr
