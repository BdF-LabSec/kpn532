...
void setup(void) {
  ...
  Serial.println("  .#####.         kirelay 0.2 (Arduino - single board)");
  ...
  Serial.println(KPN532_RELAY_RAW ? "ISO/DEP" : "RAW");
...
void loop(void) {
  ...
  if (pNFCReader->InListPassiveTarget(UID, sizeof(UID), &cbUID, SENS_RES, &SEL_RES)) {
    ...
    PN532_SPI::PrintHex(SENS_RES, sizeof(SENS_RES));
    Serial.print("|   SEL_RES : ");
    PN532_SPI::PrintHex(&SEL_RES, 1);
    Serial.print("|   UID     : ");
    PN532_SPI::PrintHex(UID, cbUID);
    ...
    if (pNFCEmulator->TgInitAsTarget(UID, cbUID, SENS_RES, SEL_RES)) {
      ...
      do {
        ...
        if (pNFCEmulator->KPN532_RELAY_GET(&pResult, &cbResult)) {
          ...
          Serial.print("< ");
          PN532_SPI::PrintHex(pResult, cbResult);
#endif
